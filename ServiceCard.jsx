    ...
    className='relative overflow-hidden max-w-lg m-2 sm:m-4 rounded-xl border 
        ...
        onMouseEnter={()=>setVisible(true)} onMouseLeave={()=>setVisible(false) } ref={divRef} onMouseMove={handleMouseMove}>
            ...
            <div className={`pointer-events-none blur-2xl rounded-full 
                bg-gradient-to-r from-blur-500 via-indigo-500 to-purple-500 
                w-[300px] h-[300px] absolute z-0 transition-opacity duration-500
                 mix-blend-lighten ${visible ? 'opacity-70' : 'opacity-0'}`} 
                 style={{top: position.y-150 , left: position.x-150}}/>

            <div className='flex items-center gap-10 p-8 hover:p-7.5 hover:m-0.5 transition-all rounded-[10px] bg-white dark:bg-gray-900 z-10 realtive'>
                ...
                <div className='flex-1'>
                    <h3 className='mt-3 mb-2 text-lg font-bold'>{service.title}</h3>
                    <p className='text-sm opacity-60 w-5/6'>{service.description}</p>
                </div>
